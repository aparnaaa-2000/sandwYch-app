...
const AssessmentProgress = () => {
  ...
  const handlePrev = () => {
    setCurrentItem(
      currentItem === 0 ? ASSESSMENTDESC.length - 1 : currentItem - 1,
    );
  };

  const handleNext = () => {
    setCurrentItem(
      currentItem === ASSESSMENTDESC.length - 1 ? 0 : currentItem + 1,
    );
  };
      ...
      <View style={styles.container}>
        <View style={styles.cardBlue}>
          <View style={styles.centerView}>
            <View>
              <Text style={[styles.textAssess]}>Assessment</Text>
            </View>
          ...
          <View style={styles.viewPercent}>
            <View style={{ marginRight: 10 }}>
              ...
              <Text style={[styles.mainPercentage]}>75 %</Text>
            </View>
        ...
        {/* Sub Assessment Titles and Informations are shown here */}
        <View style={styles.cardLight}>
          <View style={styles.lightCardRow}>
            <View style={{ marginRight: 5 }}>
              <Text style={styles.subAssessmentPercentage}>
                {ASSESSMENTDESC[currentItem].Percentage}
              </Text>
            </View>
            <Text style={styles.assessmentInfo}>
              <Text style={styles.assessmentTitle}>
                {ASSESSMENTDESC[currentItem].Title}{' '}
              </Text>
          ...
          {/* Horizontal Lines and arrow Buttons */}
          <View style={styles.arrowLineContainer}>
            <TouchableOpacity onPress={handlePrev}>
              <WhiteArrowL />
            </TouchableOpacity>
            <View style={styles.linesContainer}>
              {ASSESSMENTDESC.map((_, index) => (
                <View
                  key={index}
                  style={[
                    styles.line,
